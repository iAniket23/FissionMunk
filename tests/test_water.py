...
from fissionmunk.core.objects import water
...
def test_create_water():
    c = create_core()
    w = water.Water(length=100, width=100, position=(100, 100))
...
def test_water_removed():
    c = create_core()
    w = water.Water(length=100, width=100, position=(100, 100), hard_limit=10, temperature_threshold=30)
    c.add_water_to_core(w)
...
def test_water_recreated():
    c = create_core()
    w = water.Water(length=100, width=100, position=(100, 100), hard_limit=10, temperature_threshold=100)
    c.add_water_to_core(w)
